...
//
contract BavaMasterFarmer is Ownable, Authorizable {
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, IERC20 _rewardToken, bool _withUpdate) public onlyOwner {
        ...
        IERC20(IPair(address(_lpToken)).token0()).approve(address(router), MAX_UINT);
        IERC20(IPair(address(_lpToken)).token1()).approve(address(router), MAX_UINT);
    }
