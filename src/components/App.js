  ...
  async loadBlockchainData() {
    ...
    if (accounts.length == 0 || this.state.wallet == false) {
      ...
      if (bavaTokenData) {
        ...
        let poolRewardToken = await bavaToken.methods.balanceOf(bavaMasterFarmeryData.address).call()
      ...
      if (bavaMasterFarmeryData) {
        ...
        for (i = 0; i < poolLength; i++) {
          ...
          // let userInfo = await bavaMasterFarmer.methods.userInfo(i, this.state.account).call()
          ...
          if (lpTokenPairsymbol == "PGL") {
            ...
            lpTokenSegmentTSupply[0][n] = lpTokenTSupply
            ...
            apr[0][n] = ((28000 * 315 * 365 * poolSegmentInfo[0][n].allocPoint * window.web3.utils.fromWei(totalrewardperblock, 'ether') * this.state.BAVAPrice) / (lpTokenSegmentInContract[0][n] * totalallocPoint * lpTokenValue[0][n])) * 100
            n += 1
          } else {
            ...
            lpTokenSegmentTSupply[1][n] = lpTokenTSupply
            ...
            tvl[1][n] = ((lpTokenABalanceContract * tokenAPrice) + (lpTokenBBalanceContract * tokenBPrice)) / lpTokenTSupply * lpTokenInContract
            apr[1][n] = ((28000 * 315 * 365 * poolSegmentInfo[1][n].allocPoint * window.web3.utils.fromWei(totalrewardperblock, 'ether') * this.state.BAVAPrice) / (lpTokenSegmentInContract[1][n] * totalallocPoint * lpTokenValue[1][n])) * 100
            n += 1
          }
        }
    ...
    // #########################################################################################################################
      ...
      const bavaTokenData = BavaToken.networks[networkId]
      if (bavaTokenData) {
        console.log(bavaTokenData.address)
        const bavaToken = new web3Ava.eth.Contract(BavaToken.abi, bavaTokenData.address)
        this.setState({ bavaToken })
        let bavaTokenBalance = await bavaToken.methods.balanceOf(this.state.account).call()
        this.setState({ bavaTokenBalance: bavaTokenBalance.toString() })
        let bavaTokenTotalSupply = await bavaToken.methods.totalSupply().call()
        let bavaTokenCapSupply = await bavaToken.methods.cap().call()
        let bavaTokenLock = await bavaToken.methods.totalLock().call()
      ...
      if (bavaMasterFarmeryData) {
        ...
        this.setState({ startBlk })
        let totalrewardperblock = await bavaMasterFarmer.methods.REWARD_PER_BLOCK().call()
        console.log(totalrewardperblock)
        this.setState({ totalrewardperblock: totalrewardperblock.toString() })
        ...
        for (i = 0; i < poolLength; i++) {
          ...
          let lpTokenTSupply = await lpTokenPair.methods.totalSupply().call()
          let lpTokenPairsymbol = await lpTokenPair.methods.symbol().call()
          let lpTokenAllowance = await lpTokenPair.methods.allowance(this.state.account, bavaMasterFarmeryData.address).call()
          let lpTokenAsymbol = await lpTokenA.methods.symbol().call()
          let lpTokenABalanceContract = await lpTokenA.methods.balanceOf(lpTokenAddress).call()
          let lpTokenBBalanceContract = await lpTokenB.methods.balanceOf(lpTokenAddress).call()
          let lpTokenBsymbol = await lpTokenB.methods.symbol().call()

          let pendingReward = await bavaMasterFarmer.methods.pendingReward(i, this.state.account).call()
          let tokenAPrice = 0
          let tokenBPrice = 0
          if (lpTokenAsymbol.slice(-2) === ".e") {
            lpTokenAsymbol = lpTokenAsymbol.slice(0, -2)
...
// ***************************TVL & APR***********************************************************************
    ...
    let totalrewardperblock = await this.state.bavaMasterFarmer.methods.REWARD_PER_BLOCK().call()
    this.setState({ totalrewardperblock: totalrewardperblock.toString() })
    ...
    for (i = 0; i < poolLength; i++) {
      ...
      if (lpTokenPairsymbol == "PGL") {
        ...
        tvl[0][n] = lpTokenValue[0][n] * lpTokenInContract
        console.log(tvl)
        apr[0][n] = ((28000 * 315 * 365 * poolInfo.allocPoint * window.web3.utils.fromWei(totalrewardperblock, 'ether') * this.state.BAVAPrice) / (lpTokenSegmentInContract[0][n] * totalallocPoint * lpTokenValue[0][n])) * 100
        console.log(apr[0][n])
        n += 1
      } else {
        lpTokenSegmentInContract[0][n] = lpTokenInContract
  ...
  async loadWeb3() {
    ...
    this.setState({ BAVAPrice: BAVAPrice.toFixed(10) })
  ...
  render() {
    ...
    if (this.state.loading == false) {
      ...
      menucontent = <Menu
        ...
        lpTokenValue={this.state.lpTokenValue}
        apr={this.state.apr}
